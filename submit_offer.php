...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($name) || empty($price) || empty($description) || empty($photo) || empty($offer_type) || empty($username_offer) || empty($since_when) || empty($from_where)) {
            ...
            header("Location: index.php"); // Przekieruj z powrotem do formularza
            exit;
        }
        ...
        $stmt->execute([$name, $price, $created_by, $photo, $description, $offer_type, $username_offer, $since_when, $from_where]);
